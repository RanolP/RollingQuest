...
public class DialogManager {
		...
		for (File file : dialogFiles) {
			...
			cfg.getKeys(false).forEach(
					key -> {
						...
						dialog.setStackId(cfg.getInt(key + ".item",
								RollingQuest.defaultItem()));
						dialog.setVisible(cfg
								.getBoolean(key + ".visible", true));
	...
	public static DialogSet getDialogSet(String name) {
		List<DialogSet> filtered = dialogs.stream()
